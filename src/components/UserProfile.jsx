...
const EditProfile = (id) => {
  ...
  const [filteredData, setDonationData] = useState([]);
    ...
    const fetchDonationData = async () => {
      try {
        ...
        const useremail = decodeURIComponent(document.cookie.replace(/(?:(?:^|.*;\s*)email\s*=\s*([^;]*).*$)|^.*$/, '$1'));
         ...
         const filteredDonation = data.filter((users) => users.email === useremail);
        if (response.status === 200) {
          setDonationData(filteredDonation);
        } else {
      ...
      <div className="offcanvas-body">
        <div className="sidenav-profile">
          ...
          <div className="user-info">
            <h6 className="user-name mb-1">Go Get - All In One Donation Management System</h6>
