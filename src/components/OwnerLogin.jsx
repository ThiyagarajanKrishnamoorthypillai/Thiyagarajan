...
const OwnerLogin = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:4000/api/v1/owner/login', {
        email: email,
        password: password,
      });
      ...
      if (response.status === 200) {
          ...
          setCookie('owneremail', email, { path: '/' , sameSite: 'strict' });
          //document.cookie = `donaremail=${encodeURIComponent(email)}; path=/;`;
  ...
  return (
    <div>
      <title>Go Get App - All in one Donation</title>
        ...
        <div className="background-shape"></div>
             <div className="container">
                ...
                <img className="big-logo" src={imgfolder} alt="" ></img><p style={{color:'white'}}>Owner Login</p>                 
                        ...
                        <div className="login-meta-data"><a className="forgot-password d-block mt-3 mb-1" href="reset_password_owner">Forgot Password?</a>
                        <p className="mb-0">Didn't have an account?<Link to="/owner_register" className="ms-1" >Register Now</Link></p>
